...
stage-0:
	...
	@cd ansible && ansible-playbook -i inventory/hosts.yml stage_0.yml && cd ..
stage-1:
	@AGE_KEY_FILE=secrets/sops/keys.txt
	@cd ansible && ansible-playbook -i inventory/hosts.yml stage_1.yml && cd ..
stage-2:
	@AGE_KEY_FILE=secrets/sops/keys.txt
	@cd ansible && ansible-playbook -i inventory/hosts.yml stage_2.yml && cd ..
stage-3:
	@AGE_KEY_FILE=secrets/sops/keys.txt
	@cd terraform && terraform plan && terraform apply -auto-approve
stage-4:
	@AGE_KEY_FILE=secrets/sops/keys.txt
	@cd ansible && ansible-playbook -i inventory/hosts.yml stage_4.yml && cd ..
stage-5:
	@AGE_KEY_FILE=secrets/sops/keys.txt
	@cd ansible && ansible-playbook -i inventory/hosts.yml stage_5.yml && cd ..
stage-6:
	@AGE_KEY_FILE=secrets/sops/keys.txt
	@cd ansible && ansible-playbook -i inventory/hosts.yml stage_6.yml && cd ..
stage-7:
	@AGE_KEY_FILE=secrets/sops/keys.txt
	@cd ansible && ansible-playbook -i inventory/hosts.yml stage_7.yml && cd ..
